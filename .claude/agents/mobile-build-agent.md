...
The EAS config is at `apps/mobile/eas.json`. Available profiles:
...
All APKs are stored in `apps/mobile/appbundles/`. Create it before building:

```bash
mkdir -p /Users/asharma52/git/asharma52/hisabkitab/apps/mobile/appbundles
```
...
```bash
adb install apps/mobile/appbundles/hisabkitab-preview.apk
```

## Responsibilities

1. **Create output dir** — `mkdir -p apps/mobile/appbundles/` before every build
...
- **Build cache issues**: Run `cd apps/mobile/android && ./gradlew clean` then rebuild
