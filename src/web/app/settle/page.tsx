...
function SettleForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Redirect to friend page or home
      if (payeeId) {
        router.push(`/friends/${payeeId}`);
      } else {
    ...
    <div className="min-h-screen">
      <header className="glass-header px-4 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <Link href={payeeId ? `/friends/${payeeId}` : '/'} className="text-white/80 hover:text-white transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label htmlFor="amount" className="block text-sm font-medium text-ink mb-1.5">
                Amount <span className="text-[10px] font-bold text-ink-muted uppercase tracking-wider">({currency})</span> <span className="text-danger">*</span>
              </label>
              <input
                id="amount"
                type="number"
            ...
            <div className="flex gap-3 pt-2">
              <Link href={payeeId ? `/friends/${payeeId}` : '/'} className="btn-secondary flex-1 text-center">
                Cancel
              </Link>
              <button
                type="submit"
